...
class pdt_fntsmc_param:
    ...
    def load_param_from_yaml(self, name: str):
        ...
        self.a_s = np.array(_p['a_s']).astype(float)
        self.b1_s = np.array(_p['b1_s']).astype(float)
        self.b2_s = np.array(_p['b2_s']).astype(float)
        self.b3_s = np.array(_p['b3_s']).astype(float)
        self.k1_s = np.array(_p['k1_s']).astype(float)
        self.Ts = np.array(_p['Ts']).astype(float)
        
        self.a_c = np.array(_p['a_c']).astype(float)
        self.b1_c = np.array(_p['b1_c']).astype(float)
        self.b2_c = np.array(_p['b2_c']).astype(float)
        self.b3_c = np.array(_p['b3_c']).astype(float)
        self.k1_c = np.array(_p['k1_c']).astype(float)
...
class pdt_fntsmc_consensus(pdt_fntsmc):
    ...
    def control_update_outer_consensus(self,
        ...
        self.control_out_consensus = -(u1 + u2 + u3 + self.yyf_i + self.yyf_p + self.yyf_d + acc_com) / (d + b)
