...
class data_collector:
    def __init__(self, N: int):
        ...
        self.throttle = np.zeros((self.N, 1)).astype(float)          # 控制输入
        self.thrust = np.zeros((self.N, 1)).astype(float)         # 控制输入
    ...
    def record(self, data: dict):
        if self.index < self.N:
            ...
            self.throttle[self.index] = data['throttle']
            self.thrust[self.index] = data['thrust']
    ...
    def reset(self, N: int):
        ...
        self.throttle = np.zeros((self.N, 1)).astype(float)
        self.thrust = np.zeros((self.N, 1)).astype(float)
    ...
    def package2file(self, path: str):
        ...
        pd.DataFrame(np.hstack((self.t, self.throttle, self.thrust)),
                     columns=['time', 'throttle', 'thrust']). \
            to_csv(path + self.name[2], sep=',', index=False)

        pd.DataFrame(np.hstack((self.t, self.d_out_obs)),
                     columns=['time', 'out1_obs', 'out2_obs', 'out3_obs']). \
