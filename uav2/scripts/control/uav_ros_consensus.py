...
class UAV_ROS_Consensus:
    def __init__(self,
        ...
        self.uav_msg_2_pub = rospy.Publisher(self.group + "/uav_msg", uav_msg, queue_size=10)
    ...
    def cal_consensus_e(self, nu: np.ndarray, eta_d: np.ndarray):
        ...
        l2 = self.adj[1] * (np.array(self.uav_msg[1].eta) - np.array(self.uav_msg[1].nu)) \
            if self.uav_existance[1] == 1 else np.zeros(3)
        l3 = self.adj[2] * (self.eta() - nu)  # uav2
    ...
    def cal_consensus_de(self, dot_nu: np.ndarray, dot_eta_d: np.ndarray):
        ...
        dl2 = self.adj[1] * (np.array(self.uav_msg[1].dot_eta) - np.array(self.uav_msg[1].dot_nu)) \
            if self.uav_existance[1] == 1 else np.zeros(3)
        dl3 = self.adj[2] * (self.dot_eta() - dot_nu)
        dl4 = self.adj[3] * (np.array(self.uav_msg[3].dot_eta) - np.array(self.uav_msg[3].dot_nu)) \
            if self.uav_existance[3] == 1 else np.zeros(3)
    ...
    def check_other_uav_ok(self):
        return (self.uav_msg[0].are_you_ok.data and
                self.uav_msg[1].are_you_ok.data and
    ...
    def uav_msg_publish(self,
                        ...
                        ctrl: np.ndarray,
                        obs: np.ndarray):
        # self.uav_msg[2].are_you_ok.data = True if (self.global_flag == 2 or self.global_flag == 3) else False
        self.uav_msg[2].finish.data = True if self.global_flag == 3 else False
        self.uav_msg[2].eta = self.eta().tolist()
        self.uav_msg[2].dot_eta = self.dot_eta().tolist()
        self.uav_msg[2].ref = ref.tolist()
        self.uav_msg[2].dot_ref = dot_ref.tolist()
        self.uav_msg[2].nu = nu.tolist()
        self.uav_msg[2].dot_nu = dot_nu.tolist()
        self.uav_msg[2].dot2_nu = dot2_nu.tolist()
        self.uav_msg[2].second_order_dynamic = (-self.kt / self.m * self.dot_eta() + ctrl + obs).tolist()
        self.uav_msg[2].name = 'uav2'
        self.uav_msg_2_pub.publish(self.uav_msg[2])
