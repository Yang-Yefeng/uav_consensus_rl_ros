...
if __name__ == "__main__":
    ...
    #
    # oa = np.array(rospy.get_param('~oa_2')).astype(float)
    # op = np.array(rospy.get_param('~op_2')).astype(float)
    # oba = np.array(rospy.get_param('~oba_2')).astype(float)
    # obp = np.array(rospy.get_param('~obp_2')).astype(float)
    ...
    while not rospy.is_shutdown():
        ...
        if uav_ros.global_flag == 1:  # approaching
            ...
            if data_record.index == data_record.N:
                ...
                save_path = cur_path + '/datasave/uav2/'
