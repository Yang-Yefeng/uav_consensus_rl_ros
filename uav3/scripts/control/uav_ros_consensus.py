...
class UAV_ROS_Consensus:
    def __init__(self,
        ...
        self.uav_msg_3_pub = rospy.Publisher(self.group + "/uav_msg", uav_msg, queue_size=10)
        '''OK 标志位 pub 或者 sub'''
    ...
    def cal_consensus_e(self, nu: np.ndarray, eta_d: np.ndarray):
        ...
        l4 = self.adj[3] * (self.eta() - nu)  # uav3
        Lambda = l1 + l2 + l3 + l4
    ...
    def cal_consensus_de(self, dot_nu: np.ndarray, dot_eta_d: np.ndarray):
        ...
        dl3 = self.adj[2] * (np.array(self.uav_msg[2].dot_eta) - np.array(self.uav_msg[2].dot_nu)) \
            if self.uav_existance[2] == 1 else np.zeros(3)
        dl4 = self.adj[3] * (self.dot_eta() - dot_nu)
    ...
    def check_other_uav_ok(self):
        return (self.uav_msg[0].are_you_ok.data and
                ...
                self.uav_msg[2].are_you_ok.data)
    ...
    def uav_msg_publish(self,
                        ...
                        ctrl: np.ndarray,
                        obs: np.ndarray):
        self.uav_msg[3].finish.data = True if self.global_flag == 3 else False
        self.uav_msg[3].eta = self.eta().tolist()
        self.uav_msg[3].dot_eta = self.dot_eta().tolist()
        self.uav_msg[3].ref = ref.tolist()
        self.uav_msg[3].dot_ref = dot_ref.tolist()
        self.uav_msg[3].nu = nu.tolist()
        self.uav_msg[3].dot_nu = dot_nu.tolist()
        self.uav_msg[3].dot2_nu = dot2_nu.tolist()
        self.uav_msg[3].second_order_dynamic = (-self.kt / self.m * self.dot_eta() + ctrl + obs).tolist()
        self.uav_msg[3].name = 'uav3'
        self.uav_msg_3_pub.publish(self.uav_msg[3])
    ...
    def approaching(self):
        ...
        if ((np.linalg.norm(self.pos0 - self.pos) < 0.3) and  # 位置误差
            ...
            self.uav_msg[3].are_you_ok.data = False
            return True
        else:
